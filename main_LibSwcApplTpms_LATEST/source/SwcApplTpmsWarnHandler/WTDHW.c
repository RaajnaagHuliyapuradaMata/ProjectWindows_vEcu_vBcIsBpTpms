...
uint8 bDHW(struct LocalWarnDat* ptLWD, uint8 ucWarnCfg){
   ...
   uint8  ui8CompRes = ucPfT((uint16) (ptLWD->tSD.ushMSoll - ushMIso(ucDP_TOLERANCEc, ptLWD->tSD.scTSoll)), ushHelp);

   ptLWD->ucCurWarnLevel = aucLastPressureM1DHW[ptLWD->tHFD.tHF.ucId];
   if(1 == ucGetWarnBitWN (ptLWD->tHFD.tHF.ucId, ucDHWIxc)){
      ...
      if(ptLWD->tHFD .tHF.ucP < aucLastPressureM1DHW[ptLWD->tHFD.tHF.ucId]){
         if(ui8GradSurpassed(ptLWD->tHFD.tHF.ucId, ptLWD->tHFD.tHF.ucP) > (uint8)0){
            ...
            if(aui8DpEvCt [ptLWD->tHFD.tHF.ucId] > (uint8)0){
                    if(0 == ucGetWarnBitWN (ptLWD->tHFD.tHF.ucId,ucSFactorIxc)                                                                        ) return((uint8)0);
               else if(ptLWD->tHFD.tHF.ushVehicleSpeed  > 0                                                                                           ) return(ucSetWarnBitWN(ptLWD->tHFD.tHF.ucId, ucDHWIxc));
               else if(ptLWD->tHFD.tHF.ucP             <= ((ptLWD ->PRefMin > ucDP_TOLERANCEc) ? (ptLWD ->PRefMin - ucDP_TOLERANCEc):ucDP_TOLERANCEc) ) return(ucSetWarnBitWN(ptLWD->tHFD.tHF.ucId, ucDHWIxc));
               else                                                                                                                                     return((uint8)0);
            }
            else{
